...
eval(LetIn("x", Read(Socket), Check(Write(Net), Dfa(dfa)))) env;;
(* Exception: Failure "Write after read detected".*)

(* let x = φ[read] in write *)
eval(LetIn("x", (Check(Read(Net), Dfa(dfa))), Write(Socket))) env;;
(* - : value = Int 1 (Ok because out of scope of local policy) *)

(* let y = read in φ[let x = write in download] *)
eval(LetIn("y", Read(Socket), (Check(LetIn("x", Write(File), Download(Net)), Dfa(dfa))))) env;;
(* Exception: Failure "Write after read detected".*)

Lambda("x", Check(Read(File), Dfa(dfa)));;
Lambda("y", Write(File));;

eval (Apply(Lambda("y", Write(File)), Lambda("x", Check(Read(File), Dfa(dfa))))) env;;
...
eval(Check(LetIn("x", Read(File), (Check(LetIn("y", Write(File), Read(Net)), Dfa(dfa1)))), Dfa(dfa))) env;;
(* Failure "Read after write detected" (Ok beacuse LIFO) *)

(* let x = φ[write] in φ'[read] *) 
eval(LetIn("x", Check(Write(File), Dfa(dfa)), (Check(Read(File), Dfa(dfa1))))) env;;
(* Failure "Read after write detected" *)

(* φ[let x = read in φ'[write]] *) 
eval(Check(LetIn("x", Read(File), (Check(Write(File), Dfa(dfa1)))), Dfa(dfa))) env;;
*)
...
(* read; φ[write] *)
eval(Apply(Lambda("x", Check(Write(File), Dfa(dfa))), Read(Socket))) env;;
(* Exception: Failure "Write after read detected". *)

(* φ[read; write] *)
eval(Check(Apply(Lambda("x", Write(File)), Read(Socket)), Dfa(dfa))) env;;
(* Exception: Failure "Write after read detected". *)

(* φ[read; download; write] *)
eval (Check(Apply(Lambda("x", Write(File)), Apply(Lambda("x", Download(File)), Read(Socket))), Dfa(dfa))) env;;
(* Exception: Failure "Write after read detected". *)

(* read; φ[download]; write *)
eval (Apply(Lambda("x", Write(File)), Apply(Lambda("x", Check(Download(File), Dfa(dfa))), Read(Socket)))) env;;

(* φ[φ'[read]; write] *) 
eval (Check(Apply(Lambda("x", Write(File)), Check(Read(Socket), Dfa(dfa1))), Dfa(dfa))) env;;

eval (Check(Apply(Lambda("x", Check(Write(Socket), Dfa(dfa1))), Read(Socket)), Dfa(dfa))) env;;
