...
type exp = 
  ...
  | Ebool of bool
  | Estring of string
  | Epolicy of n_dfa list(* programmer can define policy *)
...
let rec ieval (policies: n_dfa list) (history: string)
  : (value * exp * string) =
  (* check if some policy is here *)
  match policies with (* maybe here i can remove the actual (?)*)
  ...
  | hd::tl -> (* check the current policy *)
    ...
    | Dfa dfa -> 
      ...
      | _ -> (Int 1, Epolicy(policies), history)
        end;;
