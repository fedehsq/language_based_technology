...
let eval (exp: exp) (env: 'v env): (value) = 
  ...
  (* push in head the current policy *)
  | Check(block, policy) -> 
...
come posso fare la rimozione? => fa da solo quando esce dalla ieval
...
(* FORSE POSSO METTERE NELLA CHECK NON PER FORZA UNA LISTA DI POLITICA! MA SOLO UNA POLKITICA DAL MAIN!
POI NEL PATTERN CHECK AGGIORNO METTENDO QUESTO SINGOLETTO NELLA LISTA DELLA REC EVAL*)